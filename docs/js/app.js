...
function drawLineChart(canvasId, series, legendId){
  ...
  if(!points.length){ ctx.fillStyle="#94a3b8"; ctx.fillText("No data", 12, 20); return; }
  ...
  ctx.fillText(new Date(tMin).toLocaleTimeString(), P.l, H-6);
  ctx.fillText(new Date(tMax).toLocaleTimeString(), W-110, H-6);
  ...
  series.forEach((s, i) => {
    ...
    if(leg && !leg.dataset.filled){
      // clear once
      leg.innerHTML = ""; leg.dataset.filled = "1";
    }
    if(leg){
...
function renderNews(items){
  ...
  if(!items || !items.length){
    box.innerHTML = `<div class="item"><h4>No articles yet</h4><p>Data pipeline not connected. Using demo UI.</p></div>`;
    return;
  }
  box.innerHTML = items.map(it => `
    <div class="item">
      <h4>${it.title}</h4>
      <div class="meta">${it.source || "Unknown"} • ${fmtTime(it.published)}</div>
      ${it.summary ? `<p>${it.summary}</p>` : ""}
      ${it.url ? `<div style="margin-top:6px"><a class="badge" href="${it.url}" target="_blank" rel="noopener">Read →</a></div>` : ""}
    </div>
  `).join("");
}

// try to fetch JSON; return null if 404
...
function demoSeries(labels=["USD","EUR"], start=new Date(Date.now()-6*3600e3), n=36, base=50){
  const series = labels.map((lab, i) => {
    ...
    for(let k=0;k<n;k++){
      v += (Math.random()-.5)*0.6; // soft noise
      pts.push({ t: new Date(+start + k*10*60*1000).toISOString(), v: +v.toFixed(2) });
    }
...
(async function init(){
  ...
  const modes = (config && config.modes) || ["realistic","estimated"];
  const cats  = (config && config.categories) || ["macro","politics","energy","tech"];
  ...
  async function loadAndRender(showLegendClear=true){
    ...
    // 4) try real data files (to be added in next step). If missing, use demo.
