...
# گردآوری نرخ‌ها (USD→IRR, EUR→IRR, XAU→USD) + خبرها با ادغام آرشیو ۶۰روزه
# منابع خبر: BBC Persian, Reuters World, Reuters Middle East, Al Jazeera
# خروجی: docs/data/{fx_latest.json,gold_latest.json,news_macro.json}
# پایدار در برابر خطا (در بدترین حالت خروجی fallback می‌نویسد و با exit code 0 تمام می‌شود)
...
# ---------------- Network ----------------
try:
    import requests  # نصب‌شده در GitHub Actions
except Exception:
    ...
    for url in feeds:
        try:
            ...
            for en in root.findall(f".//{ATOM_NS}entry"):
                ...
                pub = to_iso_utc(
                    (en.findtext(f"{ATOM_NS}updated") or en.findtext(f"{ATOM_NS}published") or "").strip()
                )
